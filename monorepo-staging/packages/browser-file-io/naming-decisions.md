...
|NAME-006|`getAcceptExtensions`|`createAcceptAttribute`|Provisional|Better describes the output as an HTML input `accept` attribute, not just extensions|`getAcceptExtensions`, `buildAcceptExtensions`, `formatAccept`|May remain in format-registry if package boundary says accept strings are UI helpers.|
