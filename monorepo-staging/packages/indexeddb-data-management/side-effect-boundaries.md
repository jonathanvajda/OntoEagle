...
|BOUND-011|File System Access project storage|Browser adapter|Optional browser adapter|Folder picker, permission, atomic writes, audit log.|FSA handles/files.|FSA files and handle registry.|Yes|No|No|Keep separate from IndexedDB package; integrate behind project storage backend interface.|
...
- File System Access and OPFS should be optional project backends, not part of the first IndexedDB core.
...
|REF-006|File/folder project storage is mature only in Mermaid.|Optional FSA backend.|Mermaid `storage.js`, `fsaRegistry.js`.|Permission, lock, write/read/list, migration tests.|Do not block IndexedDB package on FSA adoption.|
